...
function registrar_tramite() {
    ...
    if (txtdni.length == 0 || txtnombre.length == 0 || txtapepat.length == 0 || txtapemat.length == 0 || txtemail.length == 0 || txt_direccion.length == 0 || txt_nrodocumentos.length == 0 || txt_folios.length == 0 || txtformato.length == 0) {
        ...
        $.ajax({
            ...
            success: function (resp) {
                ...
                if (resp != 10) {
                    ...
                    if (resp != 0) {
                        if (resp == 2) {
                            Swal.fire("Mensaje de Advertencia", "Lo sentimos, el <b>nro de documento</b> ingresado ya se encuentra registrado en nuestra data o está vacio el número de documento", "warning");
                            ...
                            Swal.fire("Mensaje de Confirmaci\u00F3n", "Datos correctamente registrados,<b> nuevo documento registrado</b><br><b>Nro Seguimiento:<b><label style='color:#9B0000;'>&nbsp; " + resp + "</label><br><b>Se Envio el nro de seguimiento al correo brindado</b>", "success")
                                .then((value) => {
                                    var txt_nrodocumentos = $('#txt_nrodocumentos').val();
                                    //alert(txt_nrodocumentos);
                                    window.open("Vista/MPDF/REPORTES/imprimir_ticket_tramite.php?codigo=" + txt_nrodocumentos + "#zoom=100");
        ...
        "columns": [
            ...
            {"data": "doc_nrodocumento"},
            {
                ...
                render: function (data, type, row) {
                    if (data != 0) {
                        return row.tipodo_descripcion;
                    } else {
                        return row.tupa_descripcion;
                    }
                }
            },
...
$('#tabla_documento').on('click', '.ver_datosdocumento', function () {
    ...
    if (data.tipodocumento_id != 0) {
        $('#cmb_tipodocumento_ver').html("<option>" + cmb_tipodocumento_ver + "</option>");
    } else {
        $('#cmb_tipodocumento_ver').html("<option>" + data.tupa_descripcion + "</option>");
    }
...
$('#tabla_documento').on('click', '.ver_derivar', function () {
    ...
    area_id = data.area_id,
        ...
        cmb_tipodocumento = data.tipodocumento_id,
        txt_nrodocumentos = data.doc_nrodocumento,
        txtdni = data.doc_dniremitente,
        txt_iddocumento = data.documento_id;
    combo_area_derivar(area_id);
    //alert(data.tipodocumento_id);
    ...
    $('#cmb_tipodocumento_derivar').val(cmb_tipodocumento);
    $('#txt_nrodocumentos_derivar').val(txt_nrodocumentos);
    $('#txtdni_derivar').val(txtdni);
    var f = new Date();
            ...
            {"data": "mov_descripcion_original"},
            {
                ...
                render: function (data, type, row) {
                    if (data != "") {
                        ...
                        return "<a class='btn btn-default  btn-xs' style='color: #00b44e;' href='/Vista/documento/" + row.doc_archivo + "' target='_blank' title='Archivo no Anexado' type='button' ><i class='fas fa-2x fa-file-pdf'></i></a>";
                    }
                }
            }
        ],
...
function registrar_tramite_interno() {
    ...
    ValidacionInputRegistroTramite('txtdni', 'txtnombre', 'txtapepat', 'txtapemat', 'txtemail', 'txt_direccion', 'txt_nrodocumentos', 'txt_folios', 'txt_asunto', 'txt_ruc', 'txt_empresa', 'txt_archivo');
    if (cmb_procedenciadocumento == 0 || area_destino == 0 || txtdni.length == 0 || txtnombre.length == 0 || txtapepat.length == 0 || txtapemat.length == 0 || txtemail.length == 0 || txt_direccion.length == 0 || txt_nrodocumentos.length == 0 || txt_folios.length == 0 || txtformato.length == 0) {
        Swal.fire("Mensaje de Advertencia", "Porfavor <b>llene los campos vacios (*)</b>", "warning");
        var form = document.getElementById("form_registro_tramite");
        form.onsubmit = function (e) {
            e.preventDefault();
