...
if ($thisPost->postBlock($_POST['postID'])) {
    ...
    $cmb_tupa           = htmlspecialchars($_POST['cmb_tipodocumento'],ENT_QUOTES,'UTF-8');
	...
	if ($_FILES['txt_archivo']['tmp_name']!="") {
		...
		$archivo  = "archivo/".($cmb_tupa."-".$txt_nrodocumentos."-".$txtdni).".".$formato;
		$nombre   = "../../Vista/documento/archivo/".($cmb_tupa."-".$txt_nrodocumentos."-".$txtdni).".".$formato;
		$ruta1    = $_FILES['txt_archivo']['tmp_name'];
	}else{
		$archivo  = ""; 
	}
	
	$consulta = $MC->registrar_documento_externo($txtdni,$txtnombre,$txtapepat,$txtapemat,$txtcelular,$txtemail,$txt_direccion,$txt_ruc,$txt_empresa,$cmb_tupa,$txt_nrodocumentos,$txt_folios,$txt_asunto,$archivo,$txt_representacion);
	if ($_FILES['txt_archivo']['tmp_name']!="") {
